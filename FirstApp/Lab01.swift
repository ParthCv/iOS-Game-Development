...
class RedSquare: SCNScene {
    ...
    func setupCamera() {
        ...
        cameraNode.position = SCNVector3(0, 0, 5) // Set the position to (0, 0, 2)
    ...
    func addSquare() {
        theSquare = SCNNode(geometry: //SCNSphere(radius: 1.0)
                                    SCNPlane(width: 1, height: 1)
        ) // Create a object node of plane shape with width of 1 and height of 1
    ...
    func changeToWhite() {
        theSquare.geometry?.firstMaterial?.diffuse.contents = UIColor.white
    }
}
